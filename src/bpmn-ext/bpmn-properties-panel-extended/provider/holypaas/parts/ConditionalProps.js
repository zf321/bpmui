...
module.exports = function (group, element, bpmnFactory, translate, eventBus) {
  ...
  group.entries.push({
    ...
    html: '<div class="bpp-row">' +
      ...
      '<button class="add" data-action="add" data-show="show">' +
      '<span>X</span>' +
      '</button>' +
      '</div>' +
    ...
    add: function (element, inputNode) {
      var conditionalEventDefinition = eventDefinitionHelper.getConditionalEventDefinition(element);
      ...
      if (conditionExpression) {
        var conditionLanguage = conditionExpression.language;
        if (typeof conditionLanguage !== 'undefined') {
          expression.conditionType = 'script';
          expression.value = script.get(element, conditionExpression);
        } else {
          expression.conditionType = 'expression';
          expression.value = conditionExpression.get('body');
        }
      }
    ...
    show: function (element, inputNode) {
      return true;
    },
